        ...
        <TableBody>
          {props.userOrderItems.map((item: any) => (
            <TableRow
              ...
              <TableCell align="left" sx={{ fontWeight: 'bold' }}>${item.amount}</TableCell>
            ...
            <TableCell align="left">{item.orderDate}</TableCell>
              <TableCell align="left"><Link to={`/orderDetails/${item.orderId}`} state={{ orderData: item }}><SummarizeIcon/></Link></TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
